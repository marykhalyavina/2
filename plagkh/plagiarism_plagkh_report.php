...
$PAGE->set_url('/moodle/plagiarism/plagkh/plagiarism_plagkh_report.php', array(
    'cmid' => $cmid,
    'userid' => $userid,
...
if ($file) {
    ...
    $pagetitle = get_string('clreportpagetitle', 'plagiarism_plagkh') . ' - ' . fullname($user) . ' - ' . $filename;
} else {
    $pagetitle = get_string('clreportpagetitle', 'plagiarism_plagkh') . ' - ' . fullname($user);
}
...
// plagkh course settings.
$modulesettings = $DB->get_records_menu('plagiarism_plagkh_config', array('cm' => $cmid), '', 'name,value');

$isinstructor = plagiarism_plagkh_assignmodule::is_instructor($context);


$clmoduleenabled = plagiarism_plagkh_pluginconfig::is_plugin_configured('mod_' . $cm->modname);

// Check if plagkh plugin is disabled.
if (empty($clmoduleenabled) || empty($modulesettings['plagiarism_plagkh_enable'])) {
    echo html_writer::div(get_string('cldisabledformodule', 'plagiarism_plagkh'), null, array('style' => $errormessagestyle));
} else {
    // Incase students not allowed to see the plagiairsm score.
    if (!$isinstructor && empty($modulesettings['plagiarism_plagkh_allowstudentaccess'])) {
        echo html_writer::div(get_string('clnopageaccess', 'plagiarism_plagkh'), null, array('style' => $errormessagestyle));
    } else {
        ...
        if ($isinstructor || in_array($USER->id, $owners)) {
            ...
            if ($viewmode == 'course') {
                echo html_writer::link(
                    ...
                    get_string('clopenfullscreen', 'plagiarism_plagkh'),
                    array('title' => get_string('clopenfullscreen', 'plagiarism_plagkh'))
                );
            }

            $cl = new plagiarism_plagkh_comms();
            $scanaccesstoken = $cl->request_access_for_report($plagiarismfiles->externalid, $isinstructor);
            $lang = plagiarism_plagkh_utils::get_lang();
            echo html_writer::tag(
                ...
                array(
                    ...
                    "<form target='_self'" .
                        "method='POST'" .
                        "style='display: none;'" .
                        "action='$config->plagiarism_plagkh_apiurl/api/moodle/$config->plagiarism_plagkh_key" .
