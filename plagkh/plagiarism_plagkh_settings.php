...
if ($isadminview) {
    ...
    $PAGE->set_url('/moodle/plagiarism/plagkh/plagiarism_plagkh_settings.php');
} else {
    ...
    $PAGE->set_url('/moodle/plagiarism/plagkh/plagiarism_plagkh_settings.php', array(
        'courseid' => $courseid,
        'cmid' => $cmid,
...
if (!$isnewmodulesettings && !$isadminview && !$clmoduleenabled) {
    ...
    if (!$isinstructor) {
        ...
        if ($isinstructor) {
            ...
            if (!$isadminview) {
                $breadcrumbs = plagiarism_plagkh_utils::set_plagkh_page_navbar_breadcrumbs(
                ...
                $role = 1;
                $breadcrumbs = plagiarism_plagkh_utils::set_plagkh_page_navbar_breadcrumbs(null, null);
                $accesstoken = $cl->request_access_for_settings($role, $breadcrumbs, $modulename, null, $cmid);
            }

            $lang = plagiarism_plagkh_utils::get_lang();

            $actionurl = "$config->plagiarism_plagkh_apiurl/api/moodle/$config->plagiarism_plagkh_key/settings";
            if (isset($cmid)) {
